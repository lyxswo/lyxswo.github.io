...
git add .
git commit -m "start write a blog!"
```
### 3. 给博客添加一个主题
```bash
...
### 6. 书写博客
```bash
hugo new content/posts/my-first-blog-artic.md
```
## 第二部分：github部分
### 1. 注册github帐号
注册github帐号，并且创建仓库 `<用户名>.github.io`
`<用户名>.github.io`： 用于存放生成的静态页面

### 2. 生成 ssh 密钥
```bash
ssh-keygen -f ~/.ssh/github
```
这个命令会生成两个文件 `github` 与 `github.pub`, `github.pub` 存放的就是公钥。
在 Github 的设置中心 -> `SSH and GPG keys` 添加 `github.pub` 中的公钥
